...
client1.on("guildBanAdd", async function(guild, user) {
  ...
  guild.members.ban(entry.executor.id, {reason: "Rotwy System | İzinsiz kullanıcı yasaklama"}).catch(e => { })	
...
client1.on("guildMemberRemove", async kickhammer => {
  ...
  kickhammer.guild.members.ban(yetkili.id, {reason: "Rotwy System | izinsiz kullanıcı Kickleme!"}).catch(e => { })	
...
const entry = await member.guild
  ...
  member.guild.members.ban(entry.executor.id, {reason: "Rotwy System | İzinsiz Bot Ekleme!"}).catch(e => { })	
  member.guild.members.ban(member.id, {reason: "Rotwy System | Bot Koruma Sistemi!"}).catch(e => { })	
...
client1.on('guildUpdate', async (oldGuild, newGuild) => {
  ...
  newGuild.members.ban(entry.executor.id, {reason: "Rotwy System | URL Koruma Sistemi!"}).catch(e => { })	
...
client1.on("guildUpdate", async (oldGuild, newGuild) => {
  ...
  newGuild.members.ban(entry.executor.id, { reason: `Rotwy System | Sunucuyu izinsiz güncellemek.` }).catch(e => { })	
  const moment = require('moment');
  moment.locale('tr');
...
client2.on("roleDelete", async role => {
  ...
  role.guild.members.ban(entry.executor.id, { reason: `Rotwy System | İzinsiz rol silme!` }).catch(e => { })	
...
client2.on("roleCreate", async role => {
  ...
  role.delete({ reason: "Rotwy System | Rol Koruma Sistemi" });
...
client2.on("roleUpdate", async (oldRole, newRole) => {
  ...
  newRole.guild.members.ban(entry.executor.id, { reason: `Rotwy System | İzinsiz Rol Güncelleme!` }).catch(e => { })	
...
client3.on("channelDelete", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: `Rotwy System | İzinsiz Kanal Silme!` }).catch(e => { })	
  await channel.clone({ reason: "Rotwy System | Kanal Korum Sistemi!" }).then(async kanal => {
...
client3.on("channelCreate", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: `Rotwy System | İzinsiz Kanal Oluşturma!` }).catch(e => { })
  channel.delete({reason: "Rotwy System | Kanal Koruma Sistemi!"}).catch(e => { })
...
client3.on("channelUpdate", async (oldChannel, newChannel) => {
  ...
  newChannel.guild.members.ban(member.id, { reason: `Rotwy System | İzinsiz Kanal Güncellemek!` }).catch(e => { })
...
client3.on("webhookUpdate", async (channel) => {
  ...
  await webhooks.map(x => x.delete({reason: "Rotwy System | Webhook Silindi!"}))
  channel.guild.members.ban(entry.executor.id, {reason: "Rotwy System | İzinsiz Webhook Açmak!"})
